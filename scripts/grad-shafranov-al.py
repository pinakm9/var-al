...
class Solver:
    ...
    def __init__(self, save_folder, model_path=None) -> None:
        self.net = arch.LSTMForgetNet(50, 3, DTYPE, name='Grad-Shafranov-al')
        self.mul = arch.VanillaNet(50, 3, DTYPE, name='Grad-Shafranov-al-mul')
    ...
    def train_step(self, r, z, r_, z_, b):
        with tf.GradientTape() as tape:
            ...
            loss_m = tf.reduce_mean(boundary(self.net, r_, z_) * self.mul(r_, z_))
    ...
    @tf.function
    def train_step_mul(self, mu_0, r_, z_, b):
        with tf.GradientTape() as tape:
            L = tf.reduce_mean((self.mul(r_, z_) - mu_0 - b * boundary(self.net, r_, z_))**2)
    ...
    def learn(self, epochs=10000, n_sample=1000):
        ...
        log = {'iteration': [], 'beta': [], 'loss_a': [], 'loss_b': [], 'loss_m': [], 'loss': [], 'loss_mul': [],\
                'L2-error': [], 'objective-error': [], 'constraint-error': [], 'runtime': []}
        ...
        while epoch < epochs+1:
            for _ in range(tau):
                loss_a, loss_b, loss_m, L = self.train_step(r, z, r_, z_, b)
            mu_0 = self.mul(r_, z_)
            for _ in range(tau):
                loss_mul = self.train_step_mul(mu_0, r_, z_, b)
            if epoch % 10 == 0:
...
save_folder = '../data/Grad-Shafranov-al'
