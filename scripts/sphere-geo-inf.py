...
def boundary(u):
    ...
    return 0.#tf.sqrt((u(o*theta0) - o*p0)**2 + (u(o*theta1) - o*p1)**2)
...
def L2_error(u):
    ...
    a = u(z)
    f = u(t0)
    return tf.sqrt( tf.reduce_sum((f - a*tf.ones_like(f))**2 * w0) / tf.reduce_sum(w0) )
...
save_folder = '../data/sphere-geodesic-inf'
