...
def duration(u):
    ...
    return tf.reduce_sum(tf.sqrt((1.0 + (tf.sin(t0)*u_t)**2) / (tf.cos(alpha)-tf.cos(t0))) * w0)

# print(objective)
# print(duration(true))# == 0
# exit()
...
"""
Things to track: 1) duration or main loss, 2) boundary loss, 3) runtime, 4) iteration, 5) save every 100 steps
                 6) total loss
"""
...
class Solver:
    ...
    def __init__(self, save_folder, model_path=None) -> None:
        self.net =  arch.VanillaNet(50, 3, DTYPE, name='sphere-brac_ref')
    ...
    def learn(self, epochs=10000, n_sample=1000):
        ...
        while epoch < epochs+1:
            loss_a, loss_b, L = self.train_step(b, g)
            if epoch % 10 == 0:
...
save_folder = '../data/sphere-brac_ref'
